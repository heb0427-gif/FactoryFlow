...
public :
	...
	const int getDefectQuantity() const; // 불량 수량 반환
	const int getPassQuantity() const; // 정상 수량 반환
	const int getRemainingQuantity() const; // 남은 생산 수량 반환
	float getProgressRate() const; // 목표 수량 대비 생산 수량의 비율 백분율 반환
	const float getDefectRate() const; // 전체 생산량 대비 불량 수량 비율 백분율 반환
